...
function [best_overlap,best_boxes, best_ids] = closest_OBB(gt, boxes)
% do a matching between gt_boxes and candidates, which are 3D oriented
% bounding boxes

  gt = double(gt);
  boxes = double(boxes);
  
  num_gt_boxes = size(gt, 1);
  num_candidates = size(boxes, 1);
  
  iou_matrix = zeros(num_gt_boxes, num_candidates);
  for i = 1:num_gt_boxes
    iou = OBBOverlap_batch(gt(i,:), boxes);
    iou_matrix(i,:) = iou';
  end
  
  best_overlap = zeros(num_gt_boxes, 1);
  best_boxes = -ones(num_gt_boxes, 7);

  [best_overlap,best_boxes, best_ids] = greedy_matching(iou_matrix, gt, boxes);
end
...
function [best_overlap,best_boxes, best_ids] = greedy_matching(iou_matrix, gt_boxes, candidates)
  ...
  best_boxes = zeros(n, size(candidates,2));
  best_ids = zeros(n, 1);
